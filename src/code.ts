...
function save(
    ...
    const targetFolder: GoogleAppsScript.Drive.Folder = getFolder(name, DRIVER.getId());
    // 公開出来る内容の保存
    const doc:GoogleAppsScript.Document.Document = saveDocument(name,_date.format(DATE_FORMAT),comment,targetFolder);

    // 非公開メモ欄に入力されていた場合は、利用者のドライブにファイルを作成
    // フォルダ構成は通常メモと同構成
    if(privateComment.length > 0){
        const privateTargetFolder: GoogleAppsScript.Drive.Folder = getFolder(name, DriveApp.getRootFolder().getId());
        // 非公開の内容の保存
        const _ = saveDocument(name,_date.format(DATE_FORMAT),privateComment,privateTargetFolder);
    }
 ...
 * @param comment 保存内容
 * @param rootId 保存先ID
 * @returns 作成または更新されたドキュメントID
 */
function saveDocument(name: string, date: string, comment: string, targetFolder: GoogleAppsScript.Drive.Folder): GoogleAppsScript.Document.Document {
    let doc: GoogleAppsScript.Document.Document;
    const files = targetFolder.getFiles();
    while (files.hasNext()) {
        const file = files.next();
        if (file.getName() === date) {
...
function getFolder(name:string ,rootId:string): GoogleAppsScript.Drive.Folder{
    // 指定したディレクトリ名のディレクトリを取得
    const root = DriveApp.getFolderById(rootId);
    const folderIterator = root.getFoldersByName(name);
    // 該当ディレクトリの情報
...
function checkWhiteUser(): boolean {
    ...
    const effectiveUser = Session.getEffectiveUser().getEmail();
        const lastRow = WHITE_LIST_SHEET.getRange("A1:A")
        .getValues()
        .filter(String).length;
    const users = WHITE_LIST_SHEET.getRange(2, 1, lastRow, 1).getValues();
 ...
 * @returns 所属課リスト
 +*/
function getManager(): Array<string> {
    const managerListLastRow = SELECT_ITEM_LIST.getRange("D2:D")
        .getValues()
        .filter(String).length;
    const managerList = Array.prototype.concat.apply([], SELECT_ITEM_LIST.getRange(2, 4, managerListLastRow, 1).getValues());


    Logger.log(managerList);
    return managerList;
}