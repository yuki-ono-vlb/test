 ...
 * @returns 所属課リスト
 ...
 function getManager(): Array<string> {
    const manageristLastRow = SELECT_ITEM_LIST.getRange("D2:D")
        .getValues()
        .filter(String).length;
    const managerist = Array.prototype.concat.apply([], SELECT_ITEM_LIST.getRange(2, 4, manageristLastRow, 1).getValues());


    Logger.log(managerist);
    return managerist;
}